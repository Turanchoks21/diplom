    ...
    <Popover className="relative">
      <>
        <Popover.Button
          ...
          className="inline-flex items-center text-pale-yellow hover:text-violet-purple  
            focus:outline-none focus:ring-0 focus:ring-offset-0"
        >
          {isSolid ? (
            <SolidGlobe className="h-8" />
          ) : (
            <OutlineGlobo className="h-8" />
          )}
        </Popover.Button>
        ...
        >
          <Popover.Panel className="absolute z-10 left-1/2 transform -translate-x-1/2 mt-3 px-1 max-w-xs sm:px-0">
            <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
              <div className="relative grid gap-6 bg-white px-2 py-4 sm:gap-8 sm:p-8">
                {solutions.map((item) => (
                  <button
                    key={item.value}
